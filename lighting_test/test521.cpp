...
// camera
Camera camera(glm::vec3(0.0f, 0.0f, 3.0f));//原始
...
//glm::vec3 lightPos(1.2f, 1.0f, 2.0f);//原始
...
int main()
{
	...
	//只想使用每个顶点的前三个float，并且忽略后三个float，所以我们只需要把步长参数改成float大小的6倍就行了
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
	//纹理坐标
	...
	lampShader.use();
	lampShader.setInt("texture1",2);//==！！！对应 glActiveTexture(GL_TEXTURE2);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//===============【画盒子 boxShader或叫lightingShader】===================
		// 不要忘记首先 use 对应的着色器程序（来设定uniform）
		...
		//5_3
		lightingShader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));//!!使圆外黑暗
		...
		model3 = glm::rotate(model3, glm::radians(camera.Yaw), glm::vec3(0.0f, 0.0f, 1.0f));//航向
		model3 = glm::rotate(model3, glm::radians(camera.Pitch), glm::vec3(1.0f, 0.0f, 0.0f));//俯仰
		model3 = glm::scale(model3, glm::vec3(0.5, 2.0, 1.0));
		lampShader.setMat4("model", model3);

		//纹理
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, awesomefaceMap);//【绑定纹理！！！】
